...
This builds on top of shard-a's pipeline PR (#2) which provides:
...
| File | Role |
|------|------|
| `docker/gdb-base.Dockerfile` | Parameterized Dockerfile (from shard-a's PR #2). Takes `ARG BASE_IMAGE`, adds gdb + libtool. |
