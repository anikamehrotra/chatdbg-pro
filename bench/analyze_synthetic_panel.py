...
Two modes:
- `--filter recent`: only cells whose source sweep matches shard-a's
...
Usage:
    python -m bench.analyze_synthetic_panel --filter recent  --out shard_a_synthetic_T1_recovered
...
def collect_rows(filter_mode: str):
    ...
    for cell in sorted(PANEL.iterdir()):
        ...
        if filter_mode == "recent":
            ...
            if "shard-a-paper-final-synthetic-20260510" not in sweep:
                continue
        rows.append(r)
    return rows
...
def main() -> int:
    ...
    title = args.title or (
        "Synthetic panel — shard-a 2026-05-10 reruns only" if args.filter == "recent"
