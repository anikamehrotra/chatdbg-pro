...
Combines:
  ...
  - Fresh local sweep cells under
    `bench/results/shard-a-paper-final-realworld-20260511-T*-*/`.
...
`analyze_shard_a_realworld.py` so the two outputs can sit side-by-side.
...
LOCAL_SWEEP_RE = re.compile(
    r"^shard-a-paper-final-realworld-(?P<date>\d{8})-T(?P<tier>\d+)-"
...
def write_summary(by, out_path):
    lines = [
        ...
        "Combines fresh local shard-a sweep "
        "(`shard-a-paper-final-realworld-20260511-T*-*`) with pre-bound "
