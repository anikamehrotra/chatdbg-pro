...
**How to find it**: Run the trigger command with `strace -f -e trace=execve` and find the deepest non-shell binary in the exec chain. shard-b's `pipeline2/reprobe_buggy_binary.py` does this — run it for all bugs that have `buggy_binary_path=NULL`.
